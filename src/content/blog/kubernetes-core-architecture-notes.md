...
Enter KubernetesGoogle had already solved this internally with Borg. In 2014, Google opensourced a new project built from scratch but inspired by Borg’s lessons, calling it Kubernetes (Greek for “helmsman”, logo: a ship’s wheel). It was donated to the CNCF (Cloud Native Computing Foundation), guaranteeing vendorneutral governance. Kubernetes provides a generic orchestration layer: the same manifests and API work on AWS, GCP, Azure, DigitalOcean, or bare metal. It eliminates cloud vendor lockin for orchestration and gives you a universal, declarative platform for containerised workloads.


## 2. The Control Plane / Node Architecture at a Glance

Kubernetes is a logically centralised control plane that manages a pool of worker nodes. Every action is a cascade of watches, writes, and reconciliations.
Layer
Components
Control Plane
etcd, kubeapiserver, kubescheduler, kubecontrollermanager
Data Plane (Nodes)
kubelet, kubeproxy, container runtime (containerd/CRIO)
